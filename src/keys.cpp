...
std::map<int, KeyPair> Keys::KEYS = {
    ...
    {0x38, {"[VK_KEY_8]", "[D8]"}},
    {0x39, {"[VK_KEY_9]", "[D9]"}},
    {0x41, {"[VK_KEY_A]", "[A]","a","A","A","a","[control+A]"}},
    {0x42, {"[VK_KEY_B]", "[B]","b","B","B","b","[control+B]"}},
    {0x43, {"[VK_KEY_C]", "[C]","c","C","C","c","[control+C]"}},
    {0x44, {"[VK_KEY_D]", "[D]","d","D","D","d","[control+D]"}},
    {0x45, {"[VK_KEY_E]", "[E]","e","E","E","e","[control+E]"}},
    {0x46, {"[VK_KEY_F]", "[F]","f","F","F","f","[control+F]"}},
    {0x47, {"[VK_KEY_G]", "[G]","g","G","G","g","[control+G]"}},
    {0x48, {"[VK_KEY_H]", "[H]","h","H","H","h","[control+H]"}},
    {0x49, {"[VK_KEY_I]", "[I]","i","I","I","i","[control+I]"}},
    {0x4A, {"[VK_KEY_J]", "[J]","j","J","J","j","[control+J]"}},
    {0x4B, {"[VK_KEY_K]", "[K]","k","K","K","k","[control+K]"}},
    {0x4C, {"[VK_KEY_L]", "[L]","l","L","L","l","[control+L]"}},
    {0x4D, {"[VK_KEY_M]", "[M]","m","M","M","m","[control+M]"}},
    {0x4E, {"[VK_KEY_N]", "[N]","n","N","N","n","[control+N]"}},
    {0x4F, {"[VK_KEY_O]", "[O]","o","O","O","o","[control+O]"}},
    {0x50, {"[VK_KEY_P]", "[P]","p","P","P","p","[control+P]"}},
    {0x51, {"[VK_KEY_Q]", "[Q]","q","Q","Q","q","[control+Q]"}},
    {0x52, {"[VK_KEY_R]", "[R]","r","R","R","r","[control+R]"}},
    {0x53, {"[VK_KEY_S]", "[S]","s","S","S","s","[control+S]"}},
    {0x54, {"[VK_KEY_T]", "[T]","t","T","T","t","[control+T]"}},
    {0x55, {"[VK_KEY_U]", "[U]","u","U","U","u","[control+U]"}},
    {0x56, {"[VK_KEY_V]", "[V]","v","V","V","v","[control+V]"}},
    {0x57, {"[VK_KEY_W]", "[W]","w","W","W","w","[control+W]"}},
    {0x58, {"[VK_KEY_X]", "[X]","x","X","X","x","[control+X]"}},
    {0x59, {"[VK_KEY_Y]", "[Y]","y","Y","Y","y","[control+Y]"}},
    {0x5A, {"[VK_KEY_Z]", "[Z]","z","Z","Z","z","[control+Z]"}},
...
std::string readable_name_str(std::string vkname_string){

    if(vkname_string.at(0) == '/')
        return readable_name((int)strtol(vkname_string.c_str() + 1, nullptr, 16), false);

    return readable_name((int)strtol(vkname_string.c_str(), nullptr, 16), true);
}

std::string readable_name(int vknumber, bool pressed){
    ...
    if(control && !Keys::KEYS[vknumber].ControlName.empty())
        s = Keys::KEYS[vknumber].ControlName;

    else if(shift && capital && !Keys::KEYS[vknumber].CapShiftName.empty())
        s = Keys::KEYS[vknumber].CapShiftName;

    else if(shift && !Keys::KEYS[vknumber].ShiftName.empty())
        s = Keys::KEYS[vknumber].ShiftName;

    else if(capital && !Keys::KEYS[vknumber].CapName.empty())
        s = Keys::KEYS[vknumber].CapName;
    else if (!Keys::KEYS[vknumber].StdName.empty())
         s = Keys::KEYS[vknumber].StdName;
    else
        s = Keys::KEYS[vknumber].Name;
...
std::string readable_string(std::string str){
    ...
    for(std::size_t i=0; i<str.length(); i+=2)
    {
        ...
        if(str.at(i) == '/'){
            rs += readable_name_str(str.substr(i,3));
            i++;
        } else {
            rs += readable_name_str(str.substr(i,2));
        }
    }
    return rs;
}
...
std::string vk_string(std::string str)
{
    ...
    for(std::size_t i=0; i<str.length(); i+=2)
    {
        ...
        if(str.at(i) == '/'){
            i++;
            aux = Keys::KEYS[(int)strtol(str.substr(i,2).c_str(), nullptr, 16)].VKName;
            aux.insert(1,"/");
            rs += aux;

        } else {
            rs += Keys::KEYS[(int)strtol(str.substr(i,2).c_str(), nullptr, 16)].VKName;
        }
    }
    return rs;
}
...
std::string names_string(std::string str)
{
    ...
    for(std::size_t i=0; i<str.length(); i+=2)
    {
        ...
        if(str.at(i) == '/'){
            i++;
            aux = Keys::KEYS[(int)strtol(str.substr(i,2).c_str(), nullptr, 16)].Name;
            aux.insert(1,"/");
            rs += aux;

        } else {
            rs += Keys::KEYS[(int)strtol(str.substr(i,2).c_str(), nullptr, 16)].Name;
        }
    }
    return rs;
}
